...
public class ArgumentRelations extends MessageManager {
    ...
    @Override
    public void build() {
        this.commandManager.registerMessage(BukkitMessageKey.UNKNOWN_COMMAND, (sender, context) -> sender.sendMessage(Messages.COMMAND_NOT_FOUND.getMessage("%command%", context.getInvalidInput())));
        ...
        this.commandManager.registerMessage(BukkitMessageKey.PLAYER_ONLY, (sender, _) -> sender.sendMessage(Messages.PLAYERS_ONLY.getMessage()));

        this.commandManager.registerMessage(BukkitMessageKey.CONSOLE_ONLY, (sender, _) -> sender.sendMessage(Messages.MUST_BE_CONSOLE_SENDER.getMessage()));
    }
}